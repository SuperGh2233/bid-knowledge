...
def _ledger_for(con, *, minimum, keywords, limit: int = 50):
    """业绩清单段 —— **已参与金额筛选**（2026-09-16 用户改判），但**永不混进 `hits`**。

    **口径演变（两步，都记在这里，避免后来者只看到其中一半）**：
      ① 最初：有金额条件就**整段不返回** → 用户反馈「召回的合同还是只有 pdf，我方响应文档里
         包含合同就行」→ 改为**照样返回**、标注「未参与金额筛选」；
      ② 现在（用户：「最好让非 pdf 的合同金额也参与金额条件」）：金额门槛**下沉到业绩行**，
         用其**合同总额**参与筛选。依据是**实测**：127 份「有总额且有明细」的 CTL 合同上，
         按 1万/5万/10万/30万/50万 门槛比对，**误返 0 份、漏召最多 2 份**。

    **不随口径改变的硬约束**：
      · 业绩行**永不进入 `hits`**（它是**声明**，不是合同原件）—— 前端用带「业绩声明」小标记的
        同款卡片渲染，与合同原件同一列表但可辨；
      · **不进 `approved_documents.json`**（那份白名单语义是「已核**合同原件**」）；
      · 每条仍带 `amount_note`，写明金额是**合同总额、非产品明细金额**；
      · **未达门槛的只报数、金额未记载的单列给出文件**（不静默丢结果）。

    ⚠️ **不按查询里的「乙方」过滤业绩行**：查询里的乙方是**供应商/我方**，而业绩行的
    `party_a` 是**采购人（客户）** —— 两个角色不同名同义会误杀（实测 `乙方是欧易的代谢组合同`
    因此返回 0 条，而库里确有 9 条代谢组业绩）。业绩行只按**产品词**筛。
    """
    # 2026-09-16 口径改判（用户：「最好让非 pdf 的合同金额也参与金额条件」）：
    # 金额门槛**下沉到业绩行本身**（用其**合同总额**；实测误返 0 份，见 `locate_track_records` docstring）。
    # 未记载金额的行由检索层单列到 `excluded_no_amount`，**不静默丢弃**。
    d = locate_track_records(con, products=tuple(keywords or ()),
                             min_amount=minimum or None, limit=limit)
