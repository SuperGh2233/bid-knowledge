...
用途：`docs/llm-generation-authorization.md §7 未决` —— 方案草稿质量从未经过业务人工评审。
...
# （授权记录 docs/llm-generation-authorization.md §9：用户已配置网关并开过真实生成）。
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        print("确认在 `docs/llm-generation-authorization.md` 授权范围内后，去掉 --dry-run 再跑。")
        return 0
    ...
    print("⚠️ 以下生成会把我方响应正文发往公司网关（qwen3.7-flash）；"
          "授权记录见 docs/llm-generation-authorization.md §9。")

    # 空白评审记录表：业务同事照着勾（判定标准见 docs/proposal-quality-review.md）
    row = lambda name: f"{name:32s} ✓/✗   ✓/✗   ✓/✗   ✓/✗   ✓/✗   __ / 5   …\n"
    (out_root / "review_record.md").write_text(
        "# 方案质量业务评审记录（人工填）\n\n"
        "列：完整性 / 引用 / 数字 / 冲突 / 诚实 / 成文质量(1-5) / 备注。判定标准见 docs/proposal-quality-review.md。\n\n"
    ...
    for name, req in SAMPLES:
        ...
        # 机器可判定项汇总成 meta（评审同事只看摘要，判定标准见 docs/proposal-quality-review.md）
