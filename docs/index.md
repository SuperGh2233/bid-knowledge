...
| `../CHANGELOG.md`（仓库根） | 现行 | **已发布**的用户可见变更（每条对应一个 git tag）；开发过程见 `plans/` |
...
| 文档 | 状态 | 何时读 |
|---|---|---|
| `plans/active/PLAN-20260917-round2-feedback.md` | **active（已实施并验收）** | **第二次需求对接**：可复制正文段落（痛点主项）/ 仪器空壳行治理 / 纳税社保总金额（新 fact_type `finance_amount`）/ 生成 outline 严格标题约束；§2 实测诊断 D1–D5、§3 四项口径、§5 五步实施、**§8 验收总表（含实测数字）**、§9 第二阶段防蔓延。⚠️ **§8 的 305 passed 是当期数字**，此后需求方第三轮实测反馈的两批修复**不在本计划范围内**（见 `CHANGELOG.md` v1.4 与 `agent-handoff.md` §1/§3，期末 327 passed）|
| `plans/active/PLAN-20260921-widen-ledger-header.md` | **active（已实施，未打 tag）** | **2026-09-21 晚**：业绩/历史合同声明抽取表头判据放宽（列名空格/无序号/无当事人列）+ 重跑 408→494（+12 份真表）+ 前端文案中性化；**§4.1 完整留档**两次重跑与「误写报价表→定向清理」的事故链 |
| `plans/active/PLAN-20260921-instrument-query-fix.md` | **active（已实施，未打 tag）** | **2026-09-21 晚**：仪器型号碎片检索修复（`resolve_instrument_query` 第三路方向反转 + 词表 `DNBSEO-T7` 拼写对齐/补 `DNBSEQ` 别名），纯检索层、零库写 |
| `plans/active/PLAN-20260921-instrument-kind-gate.md` | **active（已实施，未打 tag）** | **2026-09-21 晚（系统性）**：仪器名抽取与文档分类解耦（`kind_of` 把「社保」排首位导致 92% 文档被门误挡）+ 9 类残渣规则 + 括号剥除修复 + 重跑，`instrument_name` **168→1327 条 / 68 变体**（Xenium 0→11）|
...
> **完结的计划移入 `plans/completed/`**（本目录已于 2026-09-17 首个完结计划迁入时建立）。
