...
**已结案（2026-09-17，勿重做）**：业绩清单的第二步三项候选全部用证据关闭 —— ① 受限 upsert 后缺口 **0 条**；② 227 份「候选」实为别的表（报价/简历/偏离/关联方），**不是机会**；③ 业绩表原文**不含合同号**（0 条），无法与合同原件按号对账。详见 `PLAN-20260916-track-record-search.md` §12。
...
- **数据**：`scripts/backfill_track_records.py` 把业绩清单抽取从 D1 的 29 份重跑到全部已解析响应文件 →
  ...
  内部有 305 条 —— ~~已列为第二步首要目标~~ **2026-09-17 复测：受限 upsert 后缺口 0 条，已闭环**）；
